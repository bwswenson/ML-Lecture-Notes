...
class Node:
	...
	def train(self):
		if self.depth == self.max_depth:
			return
		if len(self.attrs) == len(self.used_attrs): #BWS: If you've split on all your attributes, then quit
			return
	...
	def predict(self, attr_dict):
		...
		return self.majority_vote_result # What is this line for? It looks like it doesn't actually run typically? 
...
# 	metrics_output = sys.argv[6]
    ...
    train_data_file = 'car_train.csv'
    test_data_file = 'car_test.csv'
    max_depth = 4
    train_predict_output = 'train_out.labels'
    test_predict_output = 'test_out.labels'
    ...
    with open(train_predict_output, "w") as file:
        error_cnt = 0
